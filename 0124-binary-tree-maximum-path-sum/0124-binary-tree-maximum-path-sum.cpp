    ...
    public:
        ...
        int solve(TreeNode *root, int &ans)
        {
            ...
            int rmax = solve(root->right, ans);	//right siide max value
           	...
           	//         possible paths are : stop at currnode, currnode+left path, currnode+rightpath, currnode+left path+ currnode+ right path 
            ...
            // ans = max(ans, max(root->val, max(overall_sum, max(lmx_root, rmx_root))));
                ans=max({ans,root->val,overall_sum,lmx_root,rmx_root});
